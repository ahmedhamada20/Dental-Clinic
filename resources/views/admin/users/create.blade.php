...
<div class="container-fluid">
    <div class="row justify-content-center">
        <div class="col-lg-8">
            <div class="card">
                <div class="card-header bg-success text-white">{{ __('users.create_title') }}</div>
                <div class="card-body">
                    <form method="POST" action="{{ route('admin.users.store') }}">
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <input name="last_name" class="form-control @error('last_name') is-invalid @enderror" value="{{ old('last_name') }}" required>
                            ...
                            <div class="col-md-4">
                                ...
                                <select name="user_type" id="user_type" class="form-select @error('user_type') is-invalid @enderror" required>
                                    @foreach ($userTypes as $type)
                                        <option value="{{ $type->value }}" @selected(old('user_type') === $type->value)>{{ $type->label() }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-6">
                                <label class="form-label">{{ __('users.fields.confirm_password') }}</label>
                                <input type="password" name="password_confirmation" class="form-control" required>
                            </div>
                        </div>
                        <div class="d-flex gap-2 mt-4">
                            <button class="btn btn-success">{{ __('users.actions.create_user') }}</button>
